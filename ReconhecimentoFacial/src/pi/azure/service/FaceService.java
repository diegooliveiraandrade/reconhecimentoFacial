...
public class FaceService {
	...
	public static final String storageConnectionString = "DefaultEndpointsProtocol=https;" + "AccountName=grupopi2018;"
			+ "AccountKey=F40G06CcqLk+t7khYjocgF7rP3f/Zjqw2PKNvthrsY3jScpXICilua3BaYgFdf14DHXdVl81KuXOJMmaO94W3A==";

	private static final String faceApikey = "ecf414eaa0434da2b46426317f6b999d";
	...
	public String createPersonGroup(String name) {
		...
		try {
			...
			UriComponents builder = UriComponentsBuilder.newInstance().scheme("https")
					.host("brazilsouth.api.cognitive.microsoft.com").path("/face/v1.0/persongroups/" + name).build();

			System.out.println("Create PresonGroup Api: " + builder.toUriString());
			ResponseEntity<String> result = restTemplate.exchange(builder.toUriString(), HttpMethod.PUT, entity,
					String.class);
	...
	public String addPersonFace(String imageUrl, FaceDTO faceDto) {
		...
		try {
			...
			UriComponents builder = UriComponentsBuilder.newInstance().scheme("https")
					.host("brazilsouth.api.cognitive.microsoft.com").path("/face/v1.0/persongroups/"
							+ faceDto.getGroupId() + "/persons/" + faceDto.getPersonId() + "/persistedFaces")
					.queryParam("userData", userData).build();
	...
	public String getPerson(String groupId, String personId) {
		...
		try {
			...
			UriComponents builder = UriComponentsBuilder.newInstance().scheme("https")
					...
					.path("face/v1.0/persongroups/" + groupId + "/persons/" + personId).build();
	...
	public String getPersonFaceMetaData(String groupId, String personId, String personFaceId) {
		...
		try {
			...
			UriComponents builder = UriComponentsBuilder.newInstance().scheme("https")
					.host("brazilsouth.api.cognitive.microsoft.com").path("face/v1.0/persongroups/" + groupId
							+ "/persons/" + personId + "/persistedFaces/" + personFaceId)
					.build();
	...
	public void trainPersonGroup(String personGroupId) {
		...
		try {
			...
			UriComponents builder = UriComponentsBuilder.newInstance().scheme("https")
					.host("brazilsouth.api.cognitive.microsoft.com")
					.path("/face/v1.0/persongroups/" + personGroupId + "/train").build();
	...
	public String detectFace(String imageUrl) {
		...
		try {
			...
			UriComponents builder = UriComponentsBuilder.newInstance().scheme("https")
					...
					.queryParam("returnFaceAttributes",
	...
	public String identifyFace(String faceId, String groupId) {
		...
		try {
			...
			String body = "{ \"personGroupId\": \"" + groupId + "\", \"faceId\":[\"" + faceId + "\" ] }";
			System.out.println("file url in stored: " + body);

			HttpEntity<String> entity = new HttpEntity<String>(body, headers);

			UriComponents builder = UriComponentsBuilder.newInstance().scheme("https")
					.host("brazilsouth.api.cognitive.microsoft.com").path("/face/v1.0/identify").build();
